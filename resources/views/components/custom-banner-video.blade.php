<div style="position: relative; width: 100%; max-width: 1920px; aspect-ratio: 16 / 9; overflow: hidden;">
    <video id="localVideo" autoplay muted loop playsinline
        style="position: absolute; top: 50%; left: 50%; width: 177.77%; height: 100%; transform: translate(-50%, -50%); object-fit: cover; pointer-events: none;">
        <source src="{{ asset($video->url ?? 'path/to/default-video.mp4') }}" type="video/mp4">
...
@push('scripts')
    <script>
        // Wait ~2 seconds then hide placeholder div to reveal the video
